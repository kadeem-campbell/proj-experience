 ...
 * Searchable experience/itinerary picker instead of raw IDs.
 */
...
import { Plus, Trash2, GripVertical, ChevronDown, ChevronUp, Layers, Search, X } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          <Input placeholder="Name (e.g. Romantic Evenings)" value={newName} onChange={e => { setNewName(e.target.value); setNewSlug(e.target.value.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-')); }} />
      ...
      <div className="space-y-2">
        {collections.map((col: any) => {
          const destIds = getDestIdsForCollection(col.id);
          const destNames = destIds.map(did => destinations.find((d: any) => d.id === did)?.name).filter(Boolean);
          return (
            <Card key={col.id} className="overflow-hidden">
              <div className="flex items-center gap-2 p-3">
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpandedId(expandedId === col.id ? null : col.id)}>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {destIds.length === 0 && <Badge variant="outline" className="text-[10px] text-muted-foreground">All cities</Badge>}
                  </div>
              ...
              {expandedId === col.id && (
                <div className="border-t p-4 bg-muted/20 space-y-4">
                  ...
                  {/* Multi-city selector */}
                  <div>
                    <Label className="text-xs font-semibold">Show for these destinations</Label>
                    <p className="text-[11px] text-muted-foreground mb-2">
                      Leave all unchecked = visible everywhere. Check specific cities to restrict.
                    </p>
...
const CollectionItemsEditor = ({ collectionId, collectionType }: { collectionId: string; collectionType: string }) => {
  ...
  const { data: linkedItems = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      if (collectionType === 'experiences') {
        ...
        if (ids.length > 0) {
          const { data: itins } = await (supabase as any)
            .from('public_itineraries')
            .select('id, name')
            .in('id', ids);
          (itins || []).forEach((i: any) => { nameMap[i.id] = i.name; });
        }
        return (data || []).map((r: any) => ({ linkId: r.id, itemId: r.item_id, label: nameMap[r.item_id] || r.item_id, sub: r.item_type }));
      }
    },
  });

  // Searchable pool of experiences or itineraries
  const { data: searchPool = [] } = useQuery({
    queryKey: ['collection-search-pool', collectionType],
    queryFn: async () => {
      if (collectionType === 'experiences') {
  ...
  const handleAdd = async (itemId: string) => {
    if (collectionType === 'experiences') {
      ...
      await (supabase as any).from('collection_items').insert({ collection_id: collectionId, item_id: itemId, item_type: 'itinerary', position: linkedItems.length });
    }
    refetch();
    queryClient.invalidateQueries({ queryKey: ['home-carousels'] });
    toast({ title: 'Added' });
  ...
  return (
    <div className="border-t pt-3">
      ...
      <div className="relative">
        ...
        <Input
          placeholder={`Search ${collectionType} to add...`}
          value={searchTerm}
