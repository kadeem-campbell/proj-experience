...
/** Complete list of 249 ISO 3166-1 alpha-2 codes (countries + territories) */
...
export const CountryFlagPicker = ({ value, onSelect }: CountryFlagPickerProps) => {
  ...
  const filtered = useMemo(
    () => COUNTRIES.filter(c => c.name.toLowerCase().includes(search.toLowerCase()) || c.code.includes(search.toLowerCase())),
    [search]
  );

  // Match by code extracted from URL, not substring
  const selected = useMemo(() => {
    if (!value) return null;
    // Extract code from URL like .../flags/fr.svg
    const match = value.match(/\/flags\/([a-z]{2})\.svg/i);
    if (match) return COUNTRIES.find(c => c.code === match[1].toLowerCase());
    // Fallback: exact code match
    return COUNTRIES.find(c => value === c.code);
  }, [value]);
    ...
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {selected ? (
            <>
              ...
              <span className="text-xs text-muted-foreground uppercase ml-auto">{selected.code}</span>
            </>
          ) : value ? (
            <>
      ...
      <PopoverContent className="w-72 p-0" align="start">
        <div className="p-2 border-b border-border">
          <Input
            placeholder="Search 249 countries & territories…"
            value={search}
          ...
          <div className="p-1">
            {filtered.map(c => (
              <button
                ...
                type="button"
                onClick={() => {
                  onSelect(flagUrl(c.code), c.code.toUpperCase());
                  setOpen(false);
                  setSearch("");
                }}
                className={cn(
