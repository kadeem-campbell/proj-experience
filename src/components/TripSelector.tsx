...
import { 
  Calendar, ChevronDown, ChevronRight, Plus, Trash2, Edit2, Check, X 
} from "lucide-react";
...
export function TripSelector({
  ...
  if (trips.length === 0) {
    return (
      <div className={cn("p-4 bg-card/50 border border-border rounded-lg", className)}>
        <div className="text-center py-6">
          <Calendar className="w-10 h-10 mx-auto mb-3 text-muted-foreground" />
          <p className="text-sm text-muted-foreground mb-3">No trips created yet</p>
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className={className}>
      <div className="flex items-center justify-between px-3 py-2 bg-card/50 border border-border rounded-t-lg">
        ...
        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={onCreateTrip}>
