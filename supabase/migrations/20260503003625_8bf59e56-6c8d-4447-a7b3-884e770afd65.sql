...
DROP TRIGGER IF EXISTS trg_guard_unique_slug ON public.activity_types;
CREATE TRIGGER trg_guard_unique_slug
BEFORE INSERT OR UPDATE OF slug ON public.activity_types
FOR EACH ROW EXECUTE FUNCTION public.guard_unique_slug();

DROP TRIGGER IF EXISTS trg_auto_slug_redirect ON public.activity_types;
CREATE TRIGGER trg_auto_slug_redirect
AFTER UPDATE OF slug ON public.activity_types
...
BEGIN
  IF NEW.slug IS NULL OR OLD.slug IS NULL THEN RETURN NEW; END IF;
  ...
  IF TG_TABLE_NAME = 'products' THEN
    ...
    EXCEPTION WHEN undefined_column THEN area_changed := false;
    END;
  END IF;
  IF NOT (slug_changed OR dest_changed OR area_changed) THEN RETURN NEW; END IF;
  IF TG_TABLE_NAME = 'products' THEN
    old_path := '/things-to-do/' || COALESCE(old_dest_slug,'explore') || CASE WHEN old_area_slug IS NOT NULL THEN '/' || old_area_slug ELSE '' END || '/' || OLD.slug;
    new_path := '/things-to-do/' || COALESCE(new_dest_slug,'explore') || CASE WHEN new_area_slug IS NOT NULL THEN '/' || new_area_slug ELSE '' END || '/' || NEW.slug;
  ...
  ELSIF TG_TABLE_NAME IN ('categories','activity_types') THEN
  ...
  ON CONFLICT (source_path) DO UPDATE SET target_path = EXCLUDED.target_path, is_active = true, updated_at = now(), notes = EXCLUDED.notes;
  RETURN NEW;
END;
$function$;
